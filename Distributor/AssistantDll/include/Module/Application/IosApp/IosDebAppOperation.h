...
#define NDDEBINSTALLFULLNAME	L"/private/var/root/Media/ndDaemon/deb/ndapp.deb"
	...
	public:
		...
		//安装DEB程序
		//strAppFullName: 应用程序PC上的文件名（含路径），非DEB文件返回FALSE
		...
		//eMode			: 备份类型
		//strFileOnPc	: 备份文件存放在PC上的位置（路径+名），为扩展名为DEB
		...
		// 获取deb程序文件的元素
		// 参数：strPakage = 包的内容  strKey = 元素所对应的标示
		// 返回：元素字符串
private:
		wstring SearchDebKey(wstring strPakage, wstring strKey);
		// 根据程序的Identifier和Essential属性判断deb程序类型是user还是system
