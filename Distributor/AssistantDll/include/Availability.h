/*
    ...
    and phone OS version numbers.  For instance:
        __OSX_AVAILABLE_STARTING(__MAC_10_2,__IPHONE_1_2)
    ...
    Mac OS X and __IPHONE_OS_X_VERSION_MAX_ALLOWED = 1xxx for iPhone.  
    ...
    Examples:
        ...
        An enum available on the phone in 1.2 and later, but not available on Mac OS X:
        
            #if __IPHONE_OS_X_VERSION_MIN_REQUIRED >= __IPHONE_1_2
                enum { myEnum = 1 };
            #endif
        Note: this works because __IPHONE_OS_X_VERSION_MIN_REQUIRED is undefined and  
        evaluatesto zero when targeting the Mac OS X platform, so test becomes
        #if 0 >= 10200 which is false.
...
#ifdef __IPHONE_OS_X_VERSION_MIN_REQUIRED
