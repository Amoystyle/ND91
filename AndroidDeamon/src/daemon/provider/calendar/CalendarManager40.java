...
public class CalendarManager40  extends CalendarManagerBase{
	...
	private String CALENDAR_URI ;
	private String EVENTS_URI ;
	public String REMINDERS_URI ;
	public static final String ADDRESS = "address";
	// Calendars Field
	public static final String CALENDAR_ID = CalendarContract.Calendars._ID; //"_id";
	
	public static final String SYNC_COUNT = CalendarContract.Calendars.ACCOUNT_NAME ;//"_sync_id";
	
	public static final String NAME = CalendarContract.Calendars.NAME;//"name";
	public static final String DISPLAYNAME = CalendarContract.Calendars.CALENDAR_DISPLAY_NAME;//"calendar_displayName";
	
	public static final String HIDDEN = CalendarContract.Calendars.VISIBLE;//"visible";
	
	public static final String COLOR = CalendarContract.Calendars.CALENDAR_COLOR;//"calendar_color";
	
	//没有
	//public static final String SELECTED = "selected";
	
	public static final String TIMEZONE = CalendarContract.Calendars.CALENDAR_TIME_ZONE;//"calendar_timezone";
	public static final String ACCESS_LEVEL = CalendarContract.Calendars.CALENDAR_ACCESS_LEVEL;// "calendar_access_level";

	// Events Field
	public static final String ID = CalendarContract.Events._ID;//"_id";
	public static final String SYNC_ID = CalendarContract.Events._SYNC_ID;//"_sync_id";
	public static final String TITLE = CalendarContract.Events.TITLE;//"title";
	public static final String EVENTLOCATION = CalendarContract.Events.EVENT_LOCATION;//"eventLocation";
	public static final String DESCRIPTION = CalendarContract.Events.DESCRIPTION;//"description";
	public static final String DTSTART = CalendarContract.Events.DTSTART;//"dtstart";
	public static final String DTEND = CalendarContract.Events.DTEND;//"dtend";
	public static final String ALLDAY = CalendarContract.Events.ALL_DAY;//"allDay";
	public static final String HASALARM = CalendarContract.Events.HAS_ALARM;//"hasAlarm";
	public static final String RRULE = CalendarContract.Events.RRULE;//"rrule";
	public static final String EVENT_STATUS = CalendarContract.Events.STATUS;//"eventStatus";
	public static final String EVENT_CALENDAR_ID = CalendarContract.Events.CALENDAR_ID;//"calendar_id";
	...
	private static final String REMINDERS_WHERE =  CalendarContract.Reminders.EVENT_ID
			+ "=%d AND (" + CalendarContract.Reminders.METHOD + "=" + IReminders.METHOD_ALERT
			+ " OR " + CalendarContract.Reminders.METHOD + "=" + IReminders.METHOD_DEFAULT
			+ ")";
	//
	...
	public CalendarManager40(Context mContext) {
		this.mContext = mContext;
		CALENDAR_URI = CalendarContract.Calendars.CONTENT_URI.toString();
		try{
			mContext.getContentResolver().query(
					Uri.parse(CALENDAR_URI), null, null, null, null);
		}catch (Exception e) {
			CALENDAR_URI = "content://calendar/calendars";
		}

		EVENTS_URI = CalendarContract.Events.CONTENT_URI.toString();
		try{
			mContext.getContentResolver().query(
					Uri.parse(EVENTS_URI), null, null, null, null);
		}catch (Exception e) {
			EVENTS_URI = "content://calendar/events";
		}
		REMINDERS_URI = CalendarContract.Reminders.CONTENT_URI.toString();
		try{
			mContext.getContentResolver().query(
					Uri.parse(REMINDERS_URI), null, null, null, null);
		}catch (Exception e) {
			REMINDERS_URI = "content://calendar/reminders";
		}
				...
				try {
					...
					while (!cal_cursor.isAfterLast()) {
						...
						calendarinfo.setHidden(cal_cursor.getInt(cal_cursor
								.getColumnIndexOrThrow(HIDDEN))==1?0:1);
...
//								.getColumnIndexOrThrow(SELECTED)));
	...
	public void QueryCalendarAndEvents(ByteWriter writer) {
		String[] columns = new String[] { CALENDAR_ID, SYNC_COUNT, NAME,
				DISPLAYNAME, HIDDEN, COLOR,  TIMEZONE, ACCESS_LEVEL };
			...
			if (null != cal_cursor) {
				...
				if (cal_cursor.moveToFirst()) {
					while (!cal_cursor.isAfterLast()) {
						...
						writer.Write(cal_cursor.getInt(4)==1?0:1); // hidden
						writer.Write(cal_cursor.getInt(5)); // color
						writer.Write(1); // selected
						writer.WriteBlob(cal_cursor.getBlob(6)); // timezone
						writer.Write(cal_cursor.getInt(7)); // access_level
						cal_cursor.moveToNext();
					}
				}
			} else {
	...
	private void QueryEvents(ArrayList<Integer> calIdList, ByteWriter writer) {
		...
		String[] columns = new String[] { IEvents.CALENDAR_ID, IEvents._ID,
			    ...
			    IEvents.ORIGINAL_INSTANCE_TIME,
			...
			if (null != event_cursor) {
				...
				if (event_cursor.moveToFirst()) {
					while (!event_cursor.isAfterLast()) {
						...
						writer.WriteBlob(null); // originalEvent
						writer.Write(event_cursor.getLong(13)); // originalInstanceTime
						writer.Write(event_cursor.getLong(14)); // lastDate
						writer.WriteBlob(event_cursor.getBlob(15)); // sync_id
