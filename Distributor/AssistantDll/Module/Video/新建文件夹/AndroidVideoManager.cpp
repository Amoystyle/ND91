...
void ND91Assistant::AndroidVideoManager::Load(deque<VideoData*>& vtMyVideoList, 
...
VideoData* ND91Assistant::AndroidVideoManager::AddVideo( const wstring localVideoPath, wstring remotePath, ProgressObserver* pObserver)
{
	...
	upfileCmd.Execute();
	upfileCmd.Complete();
    //if(FileHelper::Upload(_pDevice, localVideoPath, remotePath, pObserver, pCancel))
	if(upfileCmd.GetUploadResult() == Transfer_Success)
    {        
		/*if(remotePath + CFileOperation::GetFileName(localVideoPath) != name)
		{
			DirectoryOperationCommand cmd(_pDevice);
...
bool ND91Assistant::AndroidVideoManager::DeleteVideo( VideoData* pVideoData, deque<PlayAlbumData*>& vtAlbumDataList )
{
    RETURN_FALSE_IF(!pVideoData);
    VideoOperationCommand cmd(_pDevice);
...
bool ND91Assistant::AndroidVideoManager::RenameVideo( VideoData* pVideoData, wstring strNewName )
{
    return false;
}
...
{
//    NodeData* pNode = FileHelper::GetNode(DEVICE_MANAGER->GetRootData(_pDevice)->GetDir(), pVideoData->_remotepath);//这一句会挂，原因还没找着
//    if (!pNode)
//        return false;
//    vector<NodeData*> nodeList;
//    nodeList.push_back(pNode);
//    if (FileHelper::Download(&nodeList, destDir, pObserver) == Transfer_Success)
    {
//        return pNode->GetFileToPC();
    }
...
bool ND91Assistant::AndroidVideoManager::DeletePlayAlbum( PlayAlbumData* pPlayAlbumData )
{
