...
class backref_tag : public std::pair<IterT, IterT>
{
    ...
    void REGEX_CDECL _do_print( OStreamT & sout, OtherT, ... ) const
    {
        ...
        for( IterT iter = this->first; iter != this->second; ++iter, ++iout )
            *iout = *iter;
    }
    ...
    {
        sout.write( this->first, static_cast<std::streamsize>( std::distance( this->first, this->second ) ) );
    }
...
protected:
    ...
    CUL_CLASS_DECL void _common_init( REGEX_FLAGS flags );
