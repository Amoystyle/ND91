...
WebResUrlAnalysis::WebResUrlAnalysis() 
{
}
...
void WebResUrlAnalysis::ParseDownUrl( wstring url, wstring& appname, wstring& appversion,wstring& appid)
{
	//if((int)url.find(L"Action=ReadFile")>-1)
	{
		...
		for (size_t i=0;i<splits.size();i++)
		{
			...
			{
				appname = WebResUrlAnalysis::urldecode(namevalue[1]);
			}
...
std::wstring WebResUrlAnalysis::urldecode(std::wstring decode)//utf8£¬unicodeÁ½ÖÖ±àÂë
{
	std::wstring result;
...
std::wstring WebResUrlAnalysis::urlencode( std::wstring encode )
{
...
std::wstring WebResUrlAnalysis::GetDownLoadUrl( string htmlcontent )
{
