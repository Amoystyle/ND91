...
static ND_UCHAR * __tcon_get_name_by_no( const ND_UCHAR * pszNo )
{
	...
	{		
		if( strcmp( (const char*)pszNo, _g_tcon[i].m_pNo ) == 0 )
			return (ND_UCHAR *)_g_tcon[i].m_pName;
	}
	return NULL;
}
...
static ND_UCHAR * __tcon_get_no_by_name( const ND_UCHAR * pszNo )
{
	...
	for( i = 0; i < sizeof(_g_tcon)/sizeof(_g_tcon[0]); i++ )
	{
		if( strcmp( (const char*)pszNo, _g_tcon[i].m_pName ) == 0 )
			return (ND_UCHAR *)_g_tcon[i].m_pNo;
	}
	return NULL;
}
...
static ND_UINT32 __parse_id3_head( FILE * pFile )
{
	...
	//判断头
	if( strncmp( (const char*)szTAG, (const char*)ID3_2_TAGSTR, ID3_TAG_SIZE ) == 0 )
	{
		//属于ID3_2版本
...
static ND_UINT32 __get_ID3_ver( const ND_CHAR * pszMP3FilePath )
{
	...
	fp = fopen((const char*)pszMP3FilePath, "rb");
	if (fp == NULL)
		return 0;
		
	//分析文件头
	ver = (ENUM_ID3VER)__parse_id3_head( fp );		
...
static HRESULT __parse_id3_frame_info( FILE * pMP3File, ID3Info * pID3Info )
{
	...
	iSize = iFrameSize;
	if( strncmp( (const char*)szFH, "USLT", 4 ) == 0 ||
		strncmp( (const char*)szFH, "APIC", 4 ) == 0 )
	{
		//歌词和Cover由于文件可能大，为了不消耗内存，直接存文件偏移量	
		//iOffset = _ftelli64( pMP3File );	
		iOffset = ftell( pMP3File );	
...
{
	ID3Info * p = (ID3Info*)malloc(sizeof(ID3Info));//内存泄漏
	if( p )
	{
...
//	_fseeki64( pMP3File, iPos, SEEK_SET );
	iPos = ftell( pMP3File );
	fseek( pMP3File, 0, SEEK_END );
	iSize = ftell( pMP3File );
	fseek( pMP3File, (long)iPos, SEEK_SET );
	for( i = 0; i < pID3Info->m_ID3Num; i++ )
	{
...
static HRESULT __parse_ID3_info( FILE * pMP3File, ID3Info * pID3Info, MP3AudioInfo * pMP3AudioInfo )
{
	...
	while( !feof(pMP3File) )
	{
		iPos = ftell( pMP3File );
		//读取头信息
		ver = (ENUM_ID3VER)__parse_id3_head_info( pMP3File, pID3Info );
		if( ver == 0 )
		{
...
bool Utf16ToUtf8(const unsigned char* pIn, unsigned int inSize, unsigned char** ppOut, unsigned int* pOutSize)
{
	wstring out;		// store utf8 string
	inSize -= 1;	// substract the length of BOM

#ifdef WIN32

	// Mac OS X is Big-endian OS, Windows is Little-endian OS
	if (pIn[0] == 0xFF && pIn[1] == 0xFE) //  big endian
	{
		//swab((char*)pIn + 2, pBuffer, inSize*2);
		 out = wstring((wchar_t *)pIn + 1,inSize);
	}
	else if (pIn[0] == 0xFE && pIn[1] == 0xFF)  //  little endian
	{
		out = CCodeOperation::UTF_8ToUnicode((char *)pIn + 2, inSize);
	}
	else
		return FALSE;
	...
	out = CCodeOperation::UTF_16ToUTF_8(
			(wchar_t*) pIn+2,
			inSize/2,
			bigEndian);
	...
	if (out.empty())
		return false;
	string out1 = CCodeOperation::WstringToString(out);
	unsigned char* pOut = new unsigned char[out1.size()];
	memcpy(pOut, out1.c_str(), out1.size());
...
static ND_UINT32 __get_frame_data_string( ID3Info * pID3Info, ND_UINT32 iIndex, ND_UCHAR ** ppBuffer )
{
	...
	if( pFrame )
	{
		...
		case 0:	//ASCII
			...
			p = (unsigned char*)malloc( iSize+1 );
			if( p )
			{
		...
		{
			unsigned int size;
			bool bSuccess = Utf16ToUtf8(
				(const unsigned char*)(&(pFrame->m_szData[1])),
				(pFrame->m_iFrameSize - 1)/2,
				(unsigned char**)ppBuffer,
				&size
				);
			return bSuccess ? size : 0;
		}	
		break;
...
static ND_UCHAR * __get_frame_data_block( ID3Info * pID3Info, ND_UINT32 iIndex )
{
	...
	p = (unsigned char*)malloc( pFrame->m_iFrameSize );
	if( !p ) return NULL;

	if( pFrame->m_iIsBlock )
	{		
		pFile = fopen((const char*)pID3Info->m_pOwner->m_szFilePath, "rb");
		if (pFile != NULL)
		{
	...
	if( SUCCEEDED(hr) )
	{
		...
		if( hr > 0 )
		{
			if( iDataSize != -1 )	
				...
				iSize = strlen((const char*)p)+1;
				
			//iSize = min((ND_UINT32)hr,iSize);
            iSize = ((((ND_UINT32)hr) < (iSize)) ? ((ND_UINT32)hr) : (iSize));
			if( pData )
...
static HRESULT __gen_frame_string( const ND_UCHAR * pszTag, ND_UINT32 iVer, ND_UINT32 iCodeType, void * pData, ND_UINT32 iDataSize, ID3Frame ** ppFrame )
{
	...
	if( iDataSize == -1 )
	{
		...
		case 0:
		case 3:		
			iSize = strlen((const char*)pData) + 1;			
			break;
		case 1:
		case 2:	
			iSize = ( wcslen((const wchar_t*)pData) + 1 ) * sizeof(wchar_t);		
			break;
		default:
			return E_FAIL;
		}
	...
	p = (ID3Frame*)malloc(sizeof(ID3Frame)+iSize+iLen);
	if( !p )
	{
		hr = E_OUTOFMEMORY;
		goto ret_here;
...
static HRESULT __add_frame( ID3PackPtr pPack, ID3Frame * pFrame )
{
	if( pPack->m_iFrameNum == 0 )
		pPack->m_pFrames = (ID3Frame**)malloc(sizeof(ID3FramePtr));
	else
		pPack->m_pFrames = (ID3Frame**)realloc( pPack->m_pFrames, (pPack->m_iFrameNum+1)*sizeof(ID3FramePtr) );
	if( !(pPack->m_pFrames) )
...
static HRESULT __write_frame( ID3Pack * pPack, ND_UINT32 iIndex, FILE * pOut, FILE * pIn )
{
	...
	{	//先获取数据
		...
		pData = (unsigned char*)malloc( pPack->m_pFrames[iIndex]->m_iFrameSize );
...
static HRESULT __fill_in_head( ID3Pack * pPack, FILE * pOut )
{
	...
	p = (unsigned char*)malloc( BLOCKSIZE );
	if( !p ) return E_FAIL;
	...
	{	
        ...
        i = (((BLOCKSIZE) < (iSize2)) ? (BLOCKSIZE) : (iSize2));
...
static HRESULT __write_mp3_data( FILE * pIn, FILE * pOut )
{
	...
	do
	{
		...
		if( strncmp( (const char*)szHead, "ID3", 3 ) != 0 )
		{
	...
	//复制MP3 Data
	pBuffer = (unsigned char*)malloc(BLOCKSIZE);
...
/*HANDLE*/void* ND_ParseMP3Info( const ND_CHAR * pszMP3FilePath )
{
	...
	pMP3File = fopen((const char*)pszMP3FilePath, "rb");
	if (pMP3File != NULL)	
	{
		pMP3Info = (MP3Info *)malloc(sizeof(MP3Info));
		if( pMP3Info )
		{
			...
			if( pMP3Info->m_pID3Info )
			{
				...
				strncpy( pMP3Info->m_szFilePath, pszMP3FilePath, ND_PATH_MAXSIZE );
...
HRESULT ND_GetMP3Info( /*HANDLE*/void* hMP3, ND_UINT32 iType, void * pData, ND_INT32 iDataSize )
{
	...
	case ENUM_MP3_INFO_PARA_FORMAT:						//音频格式
		{			
			if( pMP3Info->m_AudioInfo.m_iLSF == 0 )
				...
				sprintf( (char*)szBuf, "MPEG-1,Layer %d", pMP3Info->m_AudioInfo.m_iLayer );
			else if( pMP3Info->m_AudioInfo.m_iMpeg25 == 0 )
				//sprintf_s( szBuf, 256, "MPEG-2,Layer %d", pMP3Info->m_AudioInfo.m_iLayer );
				sprintf( (char*)szBuf, "MPEG-2,Layer %d", pMP3Info->m_AudioInfo.m_iLayer );
			else
				//sprintf_s( szBuf, 256, "MPEG-2.5,Layer %d", pMP3Info->m_AudioInfo.m_iLayer );
				sprintf( (char*)szBuf, "MPEG-2.5,Layer %d", pMP3Info->m_AudioInfo.m_iLayer );
			if( iDataSize != -1 )	iSize = iDataSize;
			//else iSize = strlen(szBuf)+1;
			else iSize = strlen((const char*)szBuf)+1;
			if( pData )
	...
	case ENUM_MP3_INFO_PARA_CHANNEL:					//通道
		{
			...
			case AUDIO_STEREO:	sprintf( (char*)szBuf, "立体声" ); break;
			case AUDIO_JSTEREO:	sprintf( (char*)szBuf, "联合立体声" ); break;
			case AUDIO_DUAL:	sprintf( (char*)szBuf, "双声道" ); break;
			case AUDIO_MONO:	sprintf( (char*)szBuf, "单声道" ); break;
			default:			sprintf( (char*)szBuf, "未知" ); break;
			}
			if( iDataSize != -1 )	iSize = iDataSize;
			//else iSize = strlen(szBuf)+1;
			else iSize = strlen((const char*)szBuf)+1;
	...
	case ENUM_MP3_INFO_PARA_TIT2:						//名称
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TIT2", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TPE1:						//表演者
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TPE1", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TPE2:						//专辑表演者
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TPE2", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TCOM:						//作曲者
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TCOM", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TALB:						//专辑
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TALB", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_USLT:						//歌词
		return __get_frame_block( pMP3Info->m_pID3Info, (const ND_UCHAR*)"USLT", pData, iDataSize );		
	case ENUM_MP3_INFO_PARA_APIC:						//封面
		return __get_frame_block( pMP3Info->m_pID3Info, (const ND_UCHAR*)"APIC", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TRCK:						//轨道号码
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TRCK", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TPOS:						//光盘号码
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TPOS", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TYER:						//年份
		{
			if (pMP3Info->m_pID3Info->m_pPacks[0]->m_iVer == ENUM_ID3VER_2_3_0)
				return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TYER", pData, iDataSize );
			else if (pMP3Info->m_pID3Info->m_pPacks[0]->m_iVer == ENUM_ID3VER_2_4_0)
				return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TDRC", pData, iDataSize );
			else
				return 0;
		}
	case ENUM_MP3_INFO_PARA_COMM:						//注释
		//return __get_frame_string( pMP3Info->m_pID3Info, "COMM", pData, iDataSize );
		return __get_frame_block( pMP3Info->m_pID3Info, (const ND_UCHAR*)"COMM", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TIT1:						//归类
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TIT1", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TCON:						//风格
		{
			//return __get_frame_string( pMP3Info->m_pID3Info, "TCON", pData, iDataSize );
			ND_UCHAR szBuf[128];
			ND_UCHAR * p;
			hr = __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TCON", szBuf, 128 );
			if( FAILED(hr) ) 
				return hr;
			szBuf[127] = 0;
			...
			if( iDataSize != -1 )	iSize = iDataSize;
			//else iSize = strlen(p)+1;
			else iSize = strlen((const char*)p)+1;
			if (pData)
				//strcpy_s(pData, iSize, p);
				strcpy((char*)pData, (const char*)p);
			return iSize;
		}
	case ENUM_MP3_INFO_PARA_TIT3:						//描述
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TIT3", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TBPM:						//BPM
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TBPM", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TCMP:						//加入选集
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TCMP", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOT:						//名称排序
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOT", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOA:						//专辑排序
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOA", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOP:						//表演者排序
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOP", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSO2:						//专辑表演者排序
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSO2", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOC:						//作曲者排序
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOC", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_RVAD:						//音量调整
		{
			...
			if (pMP3Info->m_pID3Info->m_pPacks[0]->m_iVer == ENUM_ID3VER_2_3_0)
				hr = __get_frame_block( pMP3Info->m_pID3Info, (const ND_UCHAR*)"RVAD", szBuf, 32 );
			else if (pMP3Info->m_pID3Info->m_pPacks[0]->m_iVer == ENUM_ID3VER_2_4_0)
				hr = __get_frame_block( pMP3Info->m_pID3Info, (const ND_UCHAR*)"RVA2", szBuf, 32 );
			else
				return 0;
			if( FAILED(hr) ) 
				return hr;
			...
			if( pData )
				memcpy( pData, &nDataTemp, iSize );
			return iSize;
		}		
	case ENUM_MP3_INFO_PARA_PCST:						//媒体种类podcast
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"PCST", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_ITNU:						//媒体种类iTunes U	
		return __get_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"ITNU", pData, iDataSize );
...
HRESULT ND_SetMP3Info( /*HANDLE*/void* hMP3, ND_UINT32 iType, void * pData, ND_INT32 iDataSize )
{
	...
	case ENUM_MP3_INFO_PARA_PATH:						//路径
		{
			...
			else iSize = strlen((const char*)pData)+1;
	...
	//	break;
	case ENUM_MP3_INFO_PARA_TIT2:						//名称
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TIT2", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TPE1:						//表演者
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TPE1", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TPE2:						//专辑表演者
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TPE2", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TCOM:						//作曲者
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TCOM", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TALB:						//专辑
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TALB", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_USLT:						//歌词
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"USLT", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_APIC:						//封面
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"APIC", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TRCK:						//轨道号码
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TRCK", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TPOS:						//光盘号码
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TPOS", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TYER:						//年份
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TYER", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_COMM:						//注释
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"COMM", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TIT1:						//归类
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TIT1", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TCON:						//风格
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TCON", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TIT3:						//描述
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TIT3", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TBPM:						//BPM
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TBPM", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TCMP:						//加入选集
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TCMP", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOT:						//名称排序
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOT", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOA:						//专辑排序
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOA", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOP:						//表演者排序
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOP", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSO2:						//专辑表演者排序
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSO2", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_TSOC:						//作曲者排序
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"TSOC", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_RVAD:						//音量调整
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"RVAD", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_PCST:						//媒体种类podcast
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"PCST", pData, iDataSize );
	case ENUM_MP3_INFO_PARA_ITNU:						//媒体种类iTunes U	
		return __set_frame_string( pMP3Info->m_pID3Info, (const ND_UCHAR*)"ITNU", pData, iDataSize );
	//case ENUM_MP3_INFO_PARA_PLAYTIME:
	//	break;
	default:
		hr = E_NOTIMPL;
...
Text encoding:
	...
	description byte directly after the frame size. If ISO-8859-1 is used
	this byte should be $00, if Unicode is used it should be $01. 

Language:
	The three byte language field is used to describe the language of the
	frame's content, according to: ISO/FDIS 639-2. Codes for the representation
	of names of languages, Part 2: Alpha-3 code

	e.g. eng is english and zho is chinese
	（ref: http://www.loc.gov/standards/iso639-2/ISO-639-2_utf-8.txt）

	FAQ: How does one make distinctions between traditional and simplified
	Chinese characters and using the ISO 639 language codes?
	...
	distinctions in script. The character sets can be coded using ISO 15924
	(Code for the Representation of Names of Scripts) script codes as subtags
	appended to the primary subtag for Chinese.

actual text:
	Unicode strings must begin with the Unicode BOM ($FF FE or $FE FF) to
	identify the byte order.

	Any empty Unicode strings which are NULL-terminated may have the Unicode
	BOM followed by a Unicode NULL ($FF FE 00 00 or $FE FF 00 00).
*/

//获取eng信息
HRESULT ND_GetEngString( void * pData, ND_UINT32 iDataSize, ND_UCHAR** ppOut, ND_UINT32* pOutSize)
{
	unsigned char* pIn = (unsigned char*)pData;
	if( *pIn == 0) // ISO-8859-1
	{
		unsigned char* pOut = new unsigned char[iDataSize];
		memset(pOut, 0, iDataSize);
		memcpy(pOut, pIn + 5, iDataSize - 5);

		*ppOut = pOut;
		*pOutSize = iDataSize - 5;
		return S_OK;
	}
	else if ( *pIn == 0x01 ) // unicode
	{
		return Utf16ToUtf8(pIn+8, iDataSize-8, ppOut, pOutSize) ? S_OK : E_FAIL;
	}
	else
		return E_FAIL;
}
