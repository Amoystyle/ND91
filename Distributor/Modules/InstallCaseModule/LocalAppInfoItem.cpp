...
{INTERFUN;
	...
	if(appinfo->m_isLegalSoft && appinfo->m_strFileType.compare(L"ipa") == 0)
	{
		...
		pLegalControl->SetAttribute(_T("bk.image"), _T("#InstallCase_Image_LegalSoftIcon"));
	...
	if (fileType.find(L"ipa")!= wstring::npos || fileType.find(L"pxl")!= wstring::npos)
	{
		...
		wstring imageId = L"#InstallCase_Image_app_iphone";
		...
		{
			imageId = L"#InstallCase_Image_app_ipadiphone";
		}
		else if (isSupposeiPhone)
		{
			imageId = L"#InstallCase_Image_app_iphone";
		}
		else if (isSupposeiPad)
		{
			imageId = L"#InstallCase_Image_app_ipad";
		}
		pSDKImage->SetAttribute(_T("bk.image"), imageId.c_str());
	}
	else
	{
		pSDKImage->SetAttribute(_T("bk.image"), _T("#InstallCase_Image_app_android"));
	}
	...
	{
		plabelFileType->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_UnKnow"));
	}
	else
	{
	...
	if(bupdate)
	{
		...
		pbuttoninstall->SetStyle(_T("ThreeBack_Button"));
		if(update)
		{
			pbuttoninstall->SetEnabled(false);
			pbuttoninstall->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_UpGrading"));
		}
		else
			pbuttoninstall->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_UpGrade"));
	...
	else
	{
		...
		pbuttoninstall->SetStyle(_T("ThreeBack_Button"));
		pbuttoninstall->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_Install"));
		pbuttoninstall->SetINotifyUI(compui);
		pcontainerBtn->Add(pbuttoninstall);

		pbuttonRemove->SetId(_T("CaseFormBtnRemove"));
		pbuttonRemove->SetInitPos(70,8,130,31,0);
		pbuttonRemove->SetStyle(_T("ThreeBack_Button"));
		pbuttonRemove->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_Delete"));
		if (busercase)
		{
			pbuttonRemove->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_Remove"));
		}
...
{INTERFUN;
	...
	if(casedata->CaseType() == _INSTALLS_CASE_DATA_IOS)
		pControl->SetAttribute(_T("bk.image"), _T("#InstallCase_Image_icon_Apple"));
	else
		pControl->SetAttribute(_T("bk.image"), _T("#InstallCase_Image_icon_Android"));
	...
	pbuttoninstall->SetInitPos(0,8,60,31,0);
	pbuttoninstall->SetStyle(_T("ThreeBack_Button"));
	pbuttoninstall->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_Show"));
	pbuttoninstall->SetINotifyUI(compui);
	pcontainerBtn->Add(pbuttoninstall);
...
{INTERFUN;
	...
	pControl->SetStyle(_T("Picture"));
	pControl->SetAttribute(_T("bk.image"), _T("#InstallCase_Image_FM_Folder"));
	pcontainerName->Add(pControl);
	...
	{
		bSys = true;
		plabelFolderName->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_AndroidFolder"));
	}
	else if(folderdata->FolderName == CONFIG(_strIosDownPath) && folderdata->Filetype == _INSTALLS_CASE_DATA_IOS)
	{
		bSys = true;
		plabelFolderName->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_IosFolder"));
	}
	else
	...
	pbuttonOpen->SetStyle(_T("ThreeBack_Button"));
	pbuttonOpen->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_Open"));
	...
	if(!bSys)
	{
		...
		pbuttonDelete->SetStyle(_T("ThreeBack_Button"));
		pbuttonDelete->SetText(CommonUIFunc::LoadString(L"InstallCase_Text_Delete"));
