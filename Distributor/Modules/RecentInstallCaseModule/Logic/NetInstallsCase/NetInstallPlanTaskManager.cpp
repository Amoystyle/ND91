...
{INTERFUN;
	...
	CStaticsticsMessage StaticMsg("52012");
	StaticMsg.Send();
	...
	for(InstallsAppList::iterator it = mNetInstallPlanItem->ListApp()->begin();it != mNetInstallPlanItem->ListApp()->end();it++)
	{
		...
		if (appinfo->m_bSelected || bNeed)
		{
			...
			RequestInstallPositionMsg requestInstallPositionMsg(mDeviceId);
...
{INTERFUN;
	...
	if (canAdd)
	{
		AnalyzeAppMsg AnalyzeMsg;
		AnalyzeMsg.filePath = savePath;
		AnalyzeMsg.SendAndGetReturn();
		appItem = (InstallsAppInfo*)AnalyzeMsg.pAppInfo;
...
void NetInstallPlanTaskManager::RunInternal(bool bNeed)
{
	...
	RefreshNetPlanCaseDataMsg RefreshMsg;
	RefreshMsg.pPlanItem = (void*)this->mNetInstallPlanItem;
	RefreshMsg.Send();

	while(!_bRefreshFinish)
	{
		Sleep(50);
	}
	// 已下载的 执行安装
	...
	if (mNetInstallPlanItem->NetInstallStatus != _NET_INSTALL_PLAN_STATUS_DOWNLOADED || mNetInstallPlanItem->NetAppItems->size() > 0)
	{
		...
		CStaticsticsMessage StaticMsg("51010");
		StaticMsg.Send();
...
void NetInstallPlanTaskManager::PlanStatus( _NET_INSTALL_PLAN_STATUS val )
{
	if (mNetInstallPlanItem->NetInstallStatus != val)
	{
		...
		{
			UpdateNetInstallPlanItemsStatusMsg UpdateStatusMsg;
			UpdateStatusMsg.planItem = (void*)mNetInstallPlanItem;
			UpdateStatusMsg.Status   = (int)val;
...
bool NetInstallPlanTaskManager::Handle(const RefreshNetPlanCaseFinishMsg* pMsg)
{
	_bRefreshFinish = true;
