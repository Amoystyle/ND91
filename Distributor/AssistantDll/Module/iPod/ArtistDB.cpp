...
wstring DArtistDB::GetTableName()
{
	if(G_DBver>=310)
...
DArtistDB* DArtistDB::FromValue(const vector<const char*>* pValues)
{
	DArtistDB* p = new DArtistDB();
	int i = 0;	
...
void DArtistDB::GetColumns(vector<wstring>& vector)
{
...
const vector<wstring>* const DArtistDB::GetValues()
{
...
wstring DArtistDB::PrimaryKeyCondition() const
{
	wostringstream woss; 
	woss<<" Pid = "<<Pid;
	return woss.str();
