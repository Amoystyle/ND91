...
namespace ND91Assistant
{
    ...
    private:
        ...
        private:
            UInt32  _type;	    // 'caff'
            UInt16  _version;
            UInt16  _flags;	    // reserved， set to 0
        };
        ...
        public:
            ...
            unsigned __int32    _formatId;          // four-character code indicating the general kind of data in the stream
            unsigned __int32    _formatFlags;       // Flags specific to each format. May be set to 0 to indicate no format flags.
            unsigned __int32    _bytesPerPacket;    // number of bytes in a packet of data. set to 0 for formats with a variable packet size
            unsigned __int32    _framesPerPacket;   // 1
            unsigned __int32    _channelsPerFrame;  // 1
            unsigned __int32    _bitsPerChannel;    // 10
        };

        class PacketTableChunkBody : public ChunkBody
        {
        public:
            PacketTableChunkBody(BinaryBufferReader* pReader, __int32 size)
            {
            }
        ...
        {
        public:
            UnknownChunkBody(BinaryBufferReader* pReader, __int32 size)
            {
        ...
        {
        public:
            DataChunkBody(BinaryBufferReader* pReader, __int32 size)
            {
        ...
        public:
            ...
            unsigned __int32    _type;	// 4byte，仅允许小写字母和空格、点
            __int64             _size;	// chunk数据区长度（若=-1，则必须是最后一个Chunk）
