...
import daemon.model.pimOms.ActionState;
import daemon.model.pimOms.Email;
import daemon.model.pimOms.GroupInfo;
import daemon.model.pimOms.GroupMemberShip;
import daemon.model.pimOms.IM;
import daemon.model.pimOms.MethodBase;
import daemon.model.pimOms.MethodKind;
import daemon.model.pimOms.OrganizationInfo;
import daemon.model.pimOms.PeopleInfo;
import daemon.model.pimOms.PhoneInfo;
import daemon.model.pimOms.PhotoInfo;
import daemon.model.pimOms.PostalAddress;
import daemon.protocol.ByteReader;
import daemon.protocol.ByteWriter;

public class ContactProviderOms_V2 implements IContactProvider_V2 {
	private Context mContext;
	//0为成功，非0失败，1：插入联系人信息失败，2：获取联系人id异常，3：插入联系人异常，4：插入email信息失败，5：插入异常
	private int mErrorCode = 0;
		
	public ContactProviderOms_V2(Context context) {
		mContext = context;
	}
	...
	private void getAllPeoples(ByteWriter writer) {
		String[] columns = new String[] {People._ID, People.DISPLAY_NAME, People.CUSTOM_RINGTONE, People.STARRED, "photo_data", "birthday", People.NOTES};
		Cursor cur = null;

		try {
			cur = mContext.getContentResolver().query(People.CONTENT_URI, columns, null, null, null);
			...
			if (null != cur && cur.moveToFirst()) {
				while (!cur.isAfterLast()) {
					...
					writer.WriteBlob(cur.getBlob(2)); // ringtone
					writer.Write(cur.getInt(3) == 1); // starred
					String photoUri = cur.getString(4);
					writer.Write(photoUri); // photo_data

					// convert uri to path
					String photoPath = getContactPhotoPath(photoUri);
					writer.Write(photoPath);

					writer.WriteBlob(cur.getBlob(5)); // birthday
					writer.WriteBlob(cur.getBlob(6)); // note
	...
	private String getContactPhotoPath(String url) {
		...
		try {
			...
			if (uri != null) {
				Cursor cursor = mContext.getContentResolver().query(uri, new String[] {Media.DATA}, null, null, null);
				if (cursor != null) {
					String path = "";
	...
	private void getAllPhones(ByteWriter writer) {
		String[] columns = new String[] {Phones._ID, Phones.PERSON_ID, Phones.TYPE, Phones.NUMBER, Phones.LABEL};
		Cursor cur = null;

		try {
			cur = mContext.getContentResolver().query(Phones.CONTENT_URI, columns, null, null, null);
			if (null != cur && cur.moveToFirst()) {
				while (!cur.isAfterLast()) {
	...
	private void getAllOrgInfos(ByteWriter writer) {
		String[] columns = new String[] {Organizations._ID, Organizations.PERSON_ID, Organizations.TYPE, Organizations.COMPANY, Organizations.TITLE, Organizations.LABEL};
		Cursor cur = null;

		try {
			cur = mContext.getContentResolver().query(Organizations.CONTENT_URI, columns, null, null, null);
			if (null != cur && cur.moveToFirst()) {
				while (!cur.isAfterLast()) {
	...
	private void getAllMemberShip(ByteWriter writer) {
		String[] columns = new String[] {GroupMembership._ID, GroupMembership.PERSON_ID, GroupMembership.GROUP_ID};
		Cursor cur = null;

		try {
			cur = mContext.getContentResolver().query(GroupMembership.CONTENT_URI, columns, null, null, null);
			if (null != cur && cur.moveToFirst()) {
				while (!cur.isAfterLast()) {
	...
	private void getAllGroups(ByteWriter writer) {
		...
		try {
			cur = mContext.getContentResolver().query(Groups.CONTENT_URI, columns, null, null, null);
	...
	private void getAllMethods(ByteWriter writer) {
		String[] columns = new String[] {ContactMethods._ID, ContactMethods.PERSON_ID, ContactMethods.KIND, ContactMethods.DATA, ContactMethods.AUX_DATA, ContactMethods.TYPE, ContactMethods.LABEL};
		Cursor cur = null;

		try {
			cur = mContext.getContentResolver().query(ContactMethods.CONTENT_URI, columns, null, null, null);
	...
	private boolean AddPeopleInfo(ArrayList<PeopleInfo> lstPeoples) {
		...
		for (int i = 0; i < peopleCnt; i++) {
			...
			if (people.getBirthday() != null && !people.getBirthday().equals(""))
				values.put("birthday", people.getBirthday());// 生日为空会出锄1�7

			values.put("photo_data", people.getPhotoUri());

			lstValues[i] = values;
		}

		try {
			int insertedCnt = mContext.getContentResolver().bulkInsert(People.CONTENT_URI, lstValues);
	...
	private ArrayList<Integer> getLatestPeopleIds(int cnt) {
		...
		cur = mContext.getContentResolver().query(People.CONTENT_URI, columns, null, null, order);
		if (cur != null) {
			if (cur.moveToFirst()) {
	...
	private ArrayList<MethodBase> getMethodsFromStream(ByteReader reader) {
		...
		while (kind != MethodKind.EndFlag) {
			if (kind == MethodKind.Email) {
				...
				ms.Read(reader);
				ms.setActionState(ActionState.Added);
				lstMethods.add(ms);
			}
	...
	private boolean PeopleDataDeal(ArrayList<MethodBase> lstData, ArrayList<Integer> insertedIds) {
		boolean result = true;
		ArrayList<MethodBase> lstMethods = new ArrayList<MethodBase>();
		...
		for (MethodBase method : lstData) {
			if (method.getKind() == MethodKind.Email || method.getKind() == MethodKind.IM || method.getKind() == MethodKind.PostalAddress) {
				...
				result = result && AddGroupMembership(ms.getContactId(), ms.getGroupId());
			}
		}
	...
	private boolean UpdatePhoneInfo(PhoneInfo phone) {
		...
		try {
			...
			values.put(Phones.LABEL, phone.getType() == 0 ? phone.getLabel() : null);

			Uri uri = ContentUris.withAppendedId(Phones.CONTENT_URI, phone.getId());
	...
	private boolean AddPhoneInfo(ArrayList<PhoneInfo> lstPhones) {
		...
		try {
			int insertedCnt = mContext.getContentResolver().bulkInsert(Phones.CONTENT_URI, lstValues);
	...
	private ArrayList<Integer> getLatestPhoneIds(int cnt) {
		...
		cur = mContext.getContentResolver().query(Phones.CONTENT_URI, columns, null, null, order);
		if (cur != null) {
			if (cur.moveToFirst()) {
				while (!cur.isAfterLast() && cnt > 0) {
					int id = cur.getInt(0);
	...
	private boolean UpdateMethodInfo(ContentValues values, Integer id) {
		...
		try {
			Uri uri = ContentUris.withAppendedId(ContactMethods.CONTENT_URI, id);
	...
	private int AddMethodInfo(ContentValues[] lstValues) {
		...
		try {
			insertedCnt = this.mContext.getContentResolver().bulkInsert(ContactMethods.CONTENT_URI, lstValues);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private boolean MethodDeal(ArrayList<MethodBase> lstMethods) {
		...
		for (MethodBase method : lstMethods) {
			...
			if (method.getKind() == MethodKind.Email) {
				...
				values.put(ContactMethods.LABEL, email.getType() == 0 ? email.getLabel() : null);
			}
				...
				values.put(ContactMethods.LABEL, im.getType() == "pre:-1" ? im.getLabel() : null);
			}
				...
				values.put(ContactMethods.LABEL, addr.getType() == 0 ? addr.getLabel() : null);
			}
			else {
				continue;
			}
		...
		if (result == true && addMethods.size() > 0) {
			result = false;
			int insertedCnt = AddMethodInfo(addValues.toArray(new ContentValues[addValues.size()]));
	...
	private ArrayList<Integer> getLatestMethodIds(int cnt) {
		...
		Cursor cur = mContext.getContentResolver().query(ContactMethods.CONTENT_URI, columns, null, null, order);
		if (cur != null) {
			if (cur.moveToFirst()) {
				while (!cur.isAfterLast() && cnt > 0) {
					int id = cur.getInt(0);
	...
	private boolean SetPhoto(Integer personId, PhotoInfo photo) {
		...
		try {
			...
			Contacts.People.setPhotoData(mContext.getContentResolver(), personUri, photo.getData());
		}
		catch (Exception e) {
			result = false;
		}
	...
	public boolean UpdateOrgInfo(OrganizationInfo org) {
		boolean result = false;
		...
		try {
			...
			values.put(Organizations.LABEL, org.getType() == 0 ? org.getLabel() : null);

			Uri uri = ContentUris.withAppendedId(Organizations.CONTENT_URI, org.getId());
	...
	public boolean AddOrgInfo(ArrayList<OrganizationInfo> lstOrg) {
		boolean result = false;
		...
		try {
			int insertedCnt = mContext.getContentResolver().bulkInsert(Organizations.CONTENT_URI, lstValues);
	...
	private ArrayList<Integer> getLatestOrgIds(int cnt) {
		...
		Cursor cur = mContext.getContentResolver().query(Organizations.CONTENT_URI, columns, null, null, order);
		if (cur != null) {
			if (cur.moveToFirst()) {
				while (!cur.isAfterLast() && cnt > 0) {
					int id = cur.getInt(0);
	...
	// 删锟斤拷锟斤拷织锟斤拷息
	public boolean DeleteOrgInfo(Integer id) {
		boolean result = false;
	...
	// 缁勭粐鐩稿叧鎿嶄綄1�7
	public boolean OrganizationDeal(ArrayList<OrganizationInfo> lstOrg) {
		boolean result = true;
	...
	private boolean UpdatePeopleInfo(PeopleInfo people) {
		...
		try {
			...
			Uri uri = ContentUris.withAppendedId(People.CONTENT_URI, people.getId());
	...
	public void GetPhoto(ByteReader reader, ByteWriter writer) {
		...
		try {
			...
			String[] columns = new String[] {Contacts.Photos._ID, Contacts.Photos.DATA};
			String where = "person=" + personId;

			writer.Write(MethodKind.Photo.getValue());

			cur = mContext.getContentResolver().query(Contacts.Photos.CONTENT_URI, columns, where, null, null);
			if (null != cur && cur.moveToFirst()) {
	...
	public void DeleteContacts(ByteReader reader, ByteWriter writer) {
		...
		try {
			...
			String where = "people._id in (" + strIds + ")";

			if (this.mContext.getContentResolver().delete(People.CONTENT_URI, where, null) != 0) {
				result = true;
			}
	...
	public void AddToGroup(ByteReader reader, ByteWriter writer) {
		...
		try {
			...
			for (int i = 0; i < peopleIds.length; i++) {
				Contacts.People.addToGroup(mContext.getContentResolver(), peopleIds[i], newGroupId);
			}
	...
	public void RemoveFromGroup(ByteReader reader, ByteWriter writer) {
		...
		try {
			...
			for (int i = 0; i < peopleIds.length; i++) {
				Uri uri = ContentUris.withAppendedId(Contacts.GroupMembership.CONTENT_URI, peopleIds[i]);
	...
	public void GetPeopleCount(ByteWriter writer) {
		...
		try {
			String[] columns = new String[] {People._ID};
			peopleCur = mContext.getContentResolver().query(People.CONTENT_URI, columns, null, null, null);
