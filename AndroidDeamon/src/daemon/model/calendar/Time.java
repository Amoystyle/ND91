...
public class Time {
	...
	/**
	 * True if this is an allDay event. The hour, minute, second fields are all
	 * zero, and the date is displayed the same in all time zones.
	 */
	...
	/**
	 * Construct a Time object in the default timezone. The time is initialized
	 * to Jan 1, 1970.
	 */
	public Time() {
	...
	/**
	 * A copy constructor. Construct a Time object by copying the given Time
	 * object. No normalization occurs.
	 * 
	 * @param other
	 */
	...
	/**
	 * Ensures the values in each field are in range. For example if the current
	 * value of this calendar is March 32, normalize() will convert it to April
	 * 1. It also fills in weekDay, yearDay, isDst and gmtoff.
	 * 
	 * <p>
	 * If "ignoreDst" is true, then this method sets the "isDst" field to -1
	 * (the "unknown" value) before normalizing. It then computes the correct
	 * value for "isDst".
	 * 
	 * <p>
	...
	public int getActualMaximum(int field) {
		...
		case YEAR_DAY: {
			...
			return ((y % 4) == 0 && ((y % 100) != 0 || (y % 400) == 0)) ? 365
					: 364;
		}
		case WEEK_NUM:
	 ...
	 * @return a String containing the current time expressed in the current
	 *         locale.
	 */
	public String format(String format) {
		synchronized (Time.class) {
			...
			if (sLocale == null || locale == null || !(locale.equals(sLocale))) {
				...
				sShortMonths = new String[] { "Jan", "Feb", "Mar", "Apr",
						"May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", };
				sLongMonths = new String[] { "January", "February", "March",
						"April", "May", "June", "July", "August", "September",
						"October", "November", "December", };
				sShortWeekdays = new String[] { "Sun", "Mon", "Tue", "Wed",
						"Thu", "Fri", "Sat", };
				sLongWeekdays = new String[] { "Sunday", "Monday", "Tuesday",
	 ...
	 * <code>yearDay</code>, and <code>gmtoff</code> are always set to zero, and
	 * the field <code>isDst</code> is set to -1 (unknown). To set those fields,
	 * call {@link #normalize(boolean)} after parsing.
	 * 
	 ...
	 * (that is, strings that contain no time or time offset). For example, all
	 * of the following strings are valid:
	 * 
	 * <ul>
	 ...
	 * {@link #allDay} field is set to true and the {@link #hour},
	 ...
	 * <em>not</em> change any of the fields in this Time object. If you want to
	 * normalize the fields in this Time object and also get the milliseconds
	 ...
	 * object. The only case where this method cannot figure out the correct
	 * value of the "isDst" field is when the time is inherently ambiguous
	 * because it falls in the hour that is repeated when switching from
	 ...
	 * this method returns, all the fields are normalized. This also sets the
	 * "isDst" field to the correct value.
	 * 
	 * @param millis
	...
	/**
	 * Sets the date from the given fields. Also sets allDay to true. Sets
	 * weekDay, yearDay and gmtoff to 0, and isDst to -1. Call
	 * {@link #normalize(boolean)} if you need those.
	 * 
	 * @param monthDay
	...
	/**
	 * Returns true if the time represented by this Time object occurs after the
	 * given time.
	 * 
	 * @param that
	 *            a given Time object to compare against
	...
	/**
	 * This array is indexed by the weekDay field (SUNDAY=0, MONDAY=1, etc.) and
	 * gives a number that can be added to the yearDay to give the closest
	 * Thursday yearDay.
	 */
	private static final int[] sThursdayOffset = { -3, 3, 2, 1, 0, -1, -2 };

	/**
	 * Computes the week number according to ISO 8601. The current Time object
	 * must already be normalized because this method uses the yearDay and
	 * weekDay fields.
	 * 
	 * <p>
	 * In IS0 8601, weeks start on Monday. The first week of the year (week 1)
	 * is defined by ISO 8601 as the first week with four or more of its days in
	 * the starting year. Or equivalently, the week containing January 4. Or
	 * equivalently, the week with the year's first Thursday in it.
	 * </p>
	 * 
	 * <p>
	 * The week number can be calculated by counting Thursdays. Week N contains
	 * the Nth Thursday of the year.
	 * </p>
	 * 
	 ...
	 * </p>
	 * <p>
	 * Otherwise, if the timezone is UTC, expresses the time as Y-M-D-T-H-M-S
	 * UTC
	 * </p>
	 * <p>
	...
	/**
	 * Returns true if the day of the given time is the epoch on the Julian
	 * Calendar (January 1, 1970 on the Gregorian calendar).
	 * 
	 * @param time
	 *            the time to test
	...
	/**
	 * Computes the Julian day number, given the UTC milliseconds and the offset
	 * (in seconds) from UTC. The Julian day for a given date will be the same
	 * for every timezone. For example, the Julian day for July 1, 2008 is
	 * 2454649. This is the same value no matter what timezone is being used.
	 * The Julian day is useful for testing if two events occur on the same day
	 * and for determining the relative time of an event from the present
	...
	public static int getJulianDay(long millis, long gmtoff) {
		...
		long julianDay = (millis + offsetMillis) / mill;
	...
	public long setJulianDay(int julianDay) {
		...
		long mill = HOUR_IN_MILLIS * 24;
		long millis = (julianDay - EPOCH_JULIAN_DAY) * mill;
		set(millis);
